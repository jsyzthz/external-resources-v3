 ...
 *   port - arduino port
 */
OnlyRGBLed::OnlyRGBLed(uint8_t port)
{
 ...
 */
OnlyRGBLed::OnlyRGBLed(uint8_t port, uint8_t led_num)
{
  pinMask       = digitalPinToBitMask(port);
  ws2812_port   = portOutputRegister(digitalPinToPort(port) );
  // set pinMode OUTPUT */
 ...
 *   None
 */
void OnlyRGBLed::setpin(uint8_t port)
{
  pinMask   = digitalPinToBitMask(port);
  ws2812_port = portOutputRegister(digitalPinToPort(port) );
  pinMode(port, OUTPUT);
 ...
 *   None
 */
void OnlyRGBLed::setNumber(uint8_t num_leds)
{
  count_led = num_leds;
 ...
 */
cRGB OnlyRGBLed::getColorAt(uint8_t index)
{
  cRGB px_value;
 ...
 *   The index value from 1 to the max
 */
uint8_t OnlyRGBLed::getNumber(void)
{
  return(count_led);
}
 ...
 */
bool OnlyRGBLed::setColorAt(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index < count_led)
  {
    uint8_t tmp = index * 3;
 ...
 */
bool OnlyRGBLed::setColor(uint8_t index, uint8_t red, uint8_t green, uint8_t blue)
{
  if(index == 0)
  {
 ...
 */
bool OnlyRGBLed::setColor(uint8_t red, uint8_t green, uint8_t blue)
{
 ...
 *   The index value from 1 to the max, if you set the index 0, all the LED will be lit
 */
bool OnlyRGBLed::setColor(uint8_t index, long value)
{
  if(index == 0)
  {
    for(int16_t i = 0; i < count_led; i++)
 ...
 *   None
 */
void OnlyRGBLed::rgbled_sendarray_mask(uint8_t *data, uint16_t datlen, uint8_t maskhi, uint8_t *port)
{
