#include "CircleBitV2IR.h"

CircleBitV2IR::CircleBitV2IR() {

}

void CircleBitV2IR::setSendPin(byte pin) {
	IR_S = pin;
	pinMode(IR_S,OUTPUT);


}
void CircleBitV2IR::setReceivePin(byte pin) {
	IR_R = pin;
	pinMode(IR_R,INPUT);

}

byte CircleBitV2IR::getByte() {
...
String CircleBitV2IR::getString() {
	irRead = getChar();
	if (irRead != 0) {
...
void CircleBitV2IR::sendString(String s) {
	uint8_t data;
	s.concat('\n');
...
void CircleBitV2IR::sendByte(byte data) {
		IR_Send38KHZ(280, 1); //发送9ms的起始码
		IR_Send38KHZ(140, 0); //发送4.5ms的结果码
		IR_Sendcode(ADD); //用户识别码
		IR_Sendcode(~ADD); //用户识别码反吗  
...
void CircleBitV2IR::IR_Send38KHZ(int x, int y) //产生38KHZ红外脉冲
		{
...
void CircleBitV2IR::IR_Sendcode(uint8_t x) //
		{
	for (int i = 0; i < 8; i++) {
		if ((x & 0x01) == 0x01) {
			IR_Send38KHZ(23, 1);
